...
- **Capability boundary:** Instagram Login is the primary path: inbound DM webhooks, comment-triggered Private Replies (exactly one per comment ID within Meta's documented windows) and window-bound replies run on `graph.instagram.com` with Instagram User tokens; Facebook Login is retained deliberately for FB-only extras and existing FB-path accounts, not as the messaging default. `comments`/`live_comments` webhooks require Advanced Access, a Live app, and public accounts (`docs/product/meta-instagram-platform-contract.md`, ADR-010; historical August-2026 research preserved in `docs/product/instagram-mvp-capability-matrix.md`, ADR-006).
