...
| Public | Landing page | Human-designated `Directam Landing — Desktop` `f6b8d46f-…-85ab43d94e44` on Page 1 | approved board source | `/` | synced from M08-007 record (fresh MCP read pending) | M08-007 |
...
| Workspace | Dashboard content | Admin Dashboard / "Dashboard — Directam Reference" `f5bf3c2c-…-8744692773fc` + states board `…8747843b4ad6` | implemented with approved primitives and real Identity/Workspace/Inbox data; standalone board mapping still pending | `/dashboard` (content) | transferred from M08-006 reconciliation (fresh MCP read pending) | M12-003 follow-up |
...
| Billing | Plans / subscription / checkout / payment results | Qasedak · Billing & Payments / "Billing / Plans / Desktop" `c48311ed-…-8820a7020aa1` (+ Current Subscription, Checkout, Payment Results, Checkout Mobile boards; provider labels updated in-file to به‌پرداخت ملت per ADR-009) | approved | `/dashboard/billing`, `/dashboard/billing/checkout`, `/dashboard/billing/result` | synced (2026-08-24; server-authoritative amounts, provider radios with Mellat disabled pending verified contract) | M09-002 UI |
