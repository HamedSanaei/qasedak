...
| ADR-012 (architecture file `docs/architecture/ADR-012-scheduled-work.md`) | Durable scheduled work is platform-owned in BuildingBlocks (`platform` schema, `scheduled_jobs`): atomic single-statement claim with leases, idempotent enqueue, deterministic backoff, dead-letter terminal states, module-owned handlers resolved per dispatch scope, no secrets in payloads | Accepted — M13-004 |
| M13-007 | Follower history is Instagram-owned (`instagram.follower_snapshots`, unique ConnectedAccountId+UTC day, provenance enum, no FK so history survives disconnect; no token/provider blob); provenance precedence Observed > Derived > Backfilled enforced by a PostgreSQL conditional upsert, never read-before-write; observed-vs-observed keeps the freshest observation, derived/backfilled keep first write; the direct absolute follower source is the IG User `followers_count` field (basic permission) because the account-insights `follower_count` metric is a documented DAILY DELTA — **historical follower backfill is NOT implemented** (no verified absolute historical series on Instagram Login; Qasedak history is durable direct observation only); daily snapshots use M13-004 with occurrence-specific account/day idempotency keys, identifier-only payloads, a next-day chain per settled occurrence and a DB-only idempotent startup bootstrap for pre-existing active accounts (bounded 500/run, no provider calls); analytics permission loss degrades only analytics, never the media catalog/follower surfaces | Accepted — M13-007 (2026-09-06) |
| M13-013 | Provider message-id uniqueness is scoped per thread: `UNIQUE (ConversationId, ProviderMessageId) WHERE ProviderMessageId IS NOT NULL` in the Conversations schema (the same provider MID can legitimately repeat across different exact accounts/threads; a global unique index was wrong and is dropped by migration `20260907053239`). Conversation-history import is channel-neutral (Instagram→composition-root bridge→Conversations `ImportProviderHistoryUseCase`), never triggers Automations (history imports create ZERO AutomationRuns — no dispatcher exists on the sync path), and records provenance/`WebhookObserved` so webhook-vs-history unread accounting stays exact. Instagram Infrastructure never writes the Conversations schema. | Accepted — M13-013 (2026-09-07) |

| PLAN-M14 | Next milestone is **M14 — Automation Operations & Safe Recovery**: prioritize workspace-scoped automation run/effect diagnostics and audited safe disposition of failed/terminal/uncertain executions; do not reopen M13, do not make external Meta approvals coding tasks, and do not weaken no-blind-resend semantics. | Accepted - planning 2026-09-08 |
