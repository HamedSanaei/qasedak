...
**Status:** DONE-PARTIAL (Zarinpal production-capable; Behpardakht Mellat live transport externally blocked pending the verified current official merchant contract)
...
**Residual (honest partial):** Live Zarinpal transport unproven against production credentials (staging smoke test requires real merchant account; deliberately not done from CI/dev machine). Behpardakht Mellat live transport cannot be implemented without the CURRENT official Behpardakht merchant technical documents listed above — boundary + config + tests only; mirrored historical manuals/community packages were deliberately not used as protocol sources.
...
**Suggested commit:** `feat(billing): integrate behpardakht mellat gateway`
